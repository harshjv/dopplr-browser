...
const debug = Debug('dopplr:service')

class DopplrService {
  constructor ({ slug, serviceUrl, defaultRpcUrl, iframeWindowRef }) {
    if (!slug || !serviceUrl || !iframeWindowRef || !defaultRpcUrl) throw new Error('Configure it properly')

    this.blocked = false
    this.emitter = new DopplrEmitter('service')
    this.slug = slug
  ...
  block () {
    ...
    this.toIframe.send({ dopplrRequestId: 'blocked' })
  }

  onRequest (d, srcWin) {
    if (!d) return

    const { dopplrRequestId, method, data } = d
    ...
    debug(`from-messenger:${method}`, dopplrRequestId, method, data)

    if (DopplrService.isAllowed(method)) {
      this.executeRequest(dopplrRequestId, method, data)
    } else {
      this.toIframe.send({
        dopplrRequestId,
  ...
  executeRequest (dopplrRequestId, method, d) {
    ...
    if (method !== 'rpc' && this.blocked) {
      this.toIframe.send({ dopplrRequestId, blocked: true, error: 'Access denied' })
      return
    }

    fn.call(this, d, (error, data) => {
      debug(`execute:${dopplrRequestId}`, method, d, error, data)

      try {
        this.toIframe.send({ dopplrRequestId, error, data })
      } catch (error) {
        debug(error)
        this.toIframe.send({ dopplrRequestId, error: 'Something went wrong :/' })
      }
    })
  }
  ...
  handleApproval (type, params, cb) {
    this.emitter.emit('dopplr-modal-open', type, params)
  ...
  signMessage (msgParams, cb) {
    this.getPrivateKey(msgParams.from, function (err, privateKey) {
      ...
      const serialized = ethUtil.bufferToHex(DopplrService.concatSignature(sig.v, sig.r, sig.s))
      cb(null, serialized)
    })
  }
...
DopplrService.isAllowed = (method) => DopplrService.allowedMethods.indexOf(method) !== -1

DopplrService.allowedMethods = [
  'getAddresses',
  'approveMessage',
  'approveTransaction',
  'signTransaction',
